<?php  
            ...
            if($_SERVER["REQUEST_METHOD"] == "POST")
            {
                ...
                {
                    $ValidateAllField = "Fillup All The Field";
                }else{
                    $ValidateAllField = "";
                }
                if(empty($name) || strlen($name)<4)
                {
                    $validateName = "Enter a valid Name";
                }
                if(empty($userName) || strlen($userName)<6 || !preg_match("/^[A-Za-z0-9_~\-!@.#\$%\^&*\(\)]+$/",$userName))
                {
                    $validateUserName = "Enter Username Correctly";
                }
                ...
                {
                    $validateDob = "Enter a Date";
                }
                
                if(empty($password) || strlen($password)<8)
                {
                    $validatePassword = "Enter a Password";
                }
                if( $password !=  $cpassword)
                {
                    $validateCPassword = "Confirm Password";
                    $password = "";
                    $cpassword = "";
                }
                if($ValidateAllField == ""){
                    
                    $dbmsg = "Succesfully Inserted";
                }
            }
